...
// The original builds its path by concatenation, so the recorder sees the name exactly
// as the code wrote it: `/srv/docs/../../etc/passwd`, not the normalized `/etc/passwd` that a
// `path.join` would have produced. Every assertion below therefore resolves before comparing.
...
function behavior() {
  ...
  assert.deepEqual(READS.map((item) => path.resolve(item)), [path.join(BASE, LEGITIMATE)]);
  process.exit(0);
}
...
function compare(originalPath, repairedPath) {
  ...
  assert.equal(path.resolve(READS[0]), '/etc/passwd');
