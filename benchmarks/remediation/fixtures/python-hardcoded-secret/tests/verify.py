...
def exploit():
    ...
    if module.API_KEY != FROM_ENVIRONMENT or LITERAL in source(local_path()):
        sys.stderr.write("vulnerability present: the API key is a literal in the source, not read from the environment\n")
        sys.exit(1)
    sys.exit(0)
...
def behavior():
    module = load(local_path())
    assert module.auth_headers()["Authorization"] == "Bearer " + module.API_KEY
    sys.exit(0)
...
def compare(original_path, repaired_path):
    ...
    assert original.API_KEY == LITERAL
    assert repaired.API_KEY == FROM_ENVIRONMENT
    assert LITERAL not in source(repaired_path)
    assert repaired.auth_headers() == {"Authorization": "Bearer " + FROM_ENVIRONMENT}
