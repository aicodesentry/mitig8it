...
// The site is a method, so reaching it means constructing the receiver first. That is exactly why
// the service declines to generate a proof for this shape and the model writes this test instead.
function run(module, email) {
  const db = recorder();
  new module.CustomerStore(db).findByEmail(email);
  return db.calls[0];
}
