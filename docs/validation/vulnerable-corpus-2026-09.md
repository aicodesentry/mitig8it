...
| remediation-service | `python -m pytest tests -q` in `services/remediation-service` | 409 passed |
| tier 2 precision benchmark | `python -m pytest tests/test_tier2_precision_benchmark.py -q` | 121 passed |
| remediation benchmark | `benchmarks/remediation/evaluate.py --suite seed` and `--adapter engine-local` | 55 cases, 43 repairs verified, 12 safe abstentions, no unexpected failures |
