...
On the 23-repository vulnerable corpus of September 2026, 249 findings are in a supported family with their file in the snapshot: 62 get a template patch, 16 get a proof, 8 have both, and all 8 verify end to end. [docs/validation/pairs-2026-09.md](../validation/pairs-2026-09.md) measures each pair on the original tree and the patched tree and says what the remaining constraint is: 84 findings get no proof because their module imports a package the dependency-free sandbox has no copy of.
