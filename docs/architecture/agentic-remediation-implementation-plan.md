...
Below each finding show: recommended fix, minimal diff, behavior preserved, verification result, coverage limits, and a manual-work reason when appropriate. At PR level show the exact batch and `Apply 3 verified fixes` / `Apply 3 fixes and merge when ready`. One click submits the already visible batch; it does not authorize any later regenerated patch.
