...
| Five families | `sql_parameterization`, `command_arguments`, `path_containment`, `hardcoded_credential` on JavaScript and Python; `code_injection_eval` on Python only. Everything else is reported per finding as `unsupported_rule_family`. The remaining JavaScript gap is the Node harness, which stubs no `eval`; it now records environment reads, which is what let `hardcoded_credential` cross over |
