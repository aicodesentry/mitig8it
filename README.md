...
| `REMEDIATION_PUBLISH_ENABLED` | Show candidates and evidence to developers, and publish each verified fix under its finding's inline review comment on GitHub: a suggestion block when the fix is one contiguous region of the finding's file that the comment can carry, otherwise the unified diff with the reason, plus the behavior-preserved line, the evidence, the coverage limitations, and a link to the app. Findings the repair service skipped get one "No automatic fix" line. Sections are keyed by candidate and updated in place. |

With generate and publish both on (the `auto_generate` capability), the API queues one remediation job for the open findings of a pull request head as soon as that head's analysis is published. The job is bounded by the policy file limit and the installation budget, at most one automatic job exists per head, and a queue failure never delays or fails the analysis. Which findings are repairable is decided by the repair service and reported per finding, so language support follows the repair service, not the API. Nothing is applied or merged automatically: a developer applies a fix with GitHub's "Apply suggestion" (a normal push that the app re-analyses, superseding the job) or with the app's per-finding Apply button.
