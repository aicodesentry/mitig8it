...
def test_tier1_still_matches_anything_short_of_containment(source):
    ...
    # Only this rule's count is this contract's business. These fixtures also trip
    # `null.pointer.deref`, quarantined separately on the September 2026 replay, so the
    # withheld set is asserted by entry rather than by equality.
    assert result["quarantined_findings"].get("path.traversal.user_path") == 1
