...
class TestRuleShape:
    ...
    def test_internal_type_is_canonical_and_not_the_check_id(self, rule):
        ...
        # The gate is the declared vocabulary, not everything derivation will accept: a
        # tier 2 rule has to name itself with a type the product chose deliberately.
        assert internal_type in DECLARED_INTERNAL_TYPES, (
            f"{rule['id']} declares {internal_type!r}, which is not in the declared set; "
            "add it to taxonomy.DECLARED_INTERNAL_TYPES deliberately or use an existing type"
        )
