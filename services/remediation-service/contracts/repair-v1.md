...
`policy.install_dependencies` (default `false`) lets the sandbox install the repository's own dependencies into the verification workspace while the snapshot is being materialized, before the first check runs. **It is implemented for the development-only local subprocess driver and nowhere else.** The isolated drivers do not install anything, and a request that asks them to is served exactly as it was before.

That is deliberate and it is a measurement rather than a plan. A dependency-free sandbox can only load a dependency-free module, and `docs/validation/pairs-2026-09.md` counted the price: 81 supported-family findings on the 23-repository corpus are refused as `dependency_not_available_in_sandbox`, against 8 that verify, every one of them a module with no imports beyond the standard library. The "With dependencies installed" section of that page is what happened when the dependencies were actually installed. 80 of the 81 modules became loadable and 7 of them produced a complete pair, and **not one of the 7 verified**: each one still failed identically on both trees. The reasons are in the page. Installing dependencies is therefore a flag the measurement needs and not a capability the isolated sandboxes have earned, and the supply-chain cost of giving a production verification run a package registry is not paid for something that converts nothing.

When it is on, the install runs only what the repository declares: `npm ci` from the lockfile the repository carries (a repository with none has one resolved first by `npm install --package-lock-only`, so the install itself is still lockfile-driven), and `pip install` from its `requirements*.txt` or, failing that, from the project itself. No package code runs: `--ignore-scripts` for npm and `--only-binary=:all:` for pip, which is pip's equivalent, so a requirement with no wheel is a recorded install failure rather than an arbitrary `setup.py`. `policy.dependency_install_timeout_seconds` (default 600) bounds the whole install and `policy.max_dependency_install_bytes` (default 2 GB) bounds what it may leave behind; over either, the installed tree is removed and the run carries the reason.

`execution_policy.network` stays `deny` and the verification level is unchanged, because installing a dependency says nothing about how isolated the run was. The local driver has no network isolation to lift or restore in the first place, and its evidence says so in a limitation rather than letting `dependencies_installed` read as an isolated driver's property.

Every run's evidence carries `evidence.dependencies`: `{dependencies_installed, ecosystems, steps, lockfile_digest, bytes_installed, duration_ms, reason_code, lockfiles, foreign_lockfiles}`. `steps` is one entry per install command with its argv, exit code, duration and output tail. `lockfile_digest` is one `sha256:` over every lockfile and requirements file the install resolved from, so two runs of the same tree at the same ref are comparable and a run whose registry handed it something else is not. `reason_code` is `null` on success and otherwise `no_dependency_manifest`, `dependency_install_timed_out`, `dependency_install_exceeded_size_cap`, `dependency_install_failed:<ecosystem>`, or `preinstalled_tree_attached`. A run that installed nothing carries `{"dependencies_installed": false}`.

With the flag on, the proof generator stops refusing `dependency_not_available_in_sandbox` for a tree that carries any `package.json`, because `npm ci` installs the lockfile's resolved closure rather than the names a manifest happens to list. A tree with no manifest anywhere is still refused, and so is TypeScript the stripper cannot handle. A module whose import the install did not in fact produce fails identically on both trees, which is recorded as an unverified pair with the install log beside it.

**The harness fakes are never shadowed by an installed package.** The proofs assert through `h.express`, `h.pg`, `h.child_process`, `h.fs` and their Python counterparts, so a workspace that now carries a real `express` or a real `psycopg` must not hand the module under test the real one. Precedence is structural and not a lookup order that could drift: the JavaScript harness answers `Module._load` from its fake table before it calls Node's loader, and its `module.registerHooks` resolver returns the fake's synthetic URL before `nextResolve`; the Python harness writes each fake into `sys.modules` before the module is executed, which is consulted before any finder on `sys.meta_path`, and replaces `os.system`, `os.popen`, `os.environ` and `open` on the real objects. `tests/test_dependency_installed_verification.py` holds this with a real package of each faked name installed beside the module.
